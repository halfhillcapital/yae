...
- `types.ts` - Type definitions (`GraphNode`, `Branch`, `Chainable`, `Action`, `NextAction`, `RetryConfig`, `isBranch`)
...
```ts
// Create typed factories - bind state type once, use everywhere
...
- `createNodes<S>()` - Returns `{ node, parallel }` factories bound to state type S
...
Flows implement `GraphNode`, so they can be chained like regular nodes:

```ts
// Define reusable flows
const authFlow = Flow.from(chain(validateToken, refreshIfExpired));
const dataFlow = Flow.from(chain(fetchData, transform, save));

// Chain flows together
const pipeline = chain(authFlow, dataFlow);

// Or wrap in outer flow with hooks
const app = Flow.from(chain(authFlow, dataFlow), {
  beforeStart: (s) => console.log("Starting pipeline..."),
  afterComplete: (s) => console.log("Pipeline complete"),
});

await app.run(shared);
```
...
- `index.ts` - Types, utilities (`defineWorkflow`, `createWorkflow`, `createWorkflowNodes`), and `runWorkflow`
...
**`createWorkflow()` + `createWorkflowNodes()` for composable workflows:**

Use `createWorkflowNodes<T>()` to define nodes separately, then compose with standard graph utilities:

```ts
import { createWorkflowNodes, createWorkflow } from "@yae/core/workflows";
import { Flow, chain } from "@yae/graph";

// Create typed factories - bind data type once, use everywhere
